...
bool Controller::Calculate(const char* input, const char* input_x) {
  ...
  if (return_value) result = model->GetResult();
  return return_value;
}
...
bool Controller::CheckGraph(const char* x_min_char_str,
  ...
  return model->CheckGraph(x_min_char_str, x_max_char_str, y_min_char_str,
                           y_max_char_str, step_char_str, x_min, x_max, y_min,
                           y_max, step);
}
}  // namespace s21
